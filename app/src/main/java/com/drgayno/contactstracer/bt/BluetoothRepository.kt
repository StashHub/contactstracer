package com.drgayno.contactstracer.bt
...
import com.drgayno.contactstracer.db.ScanDataEntity
import com.drgayno.contactstracer.db.SharedPrefsRepository
import com.drgayno.contactstracer.ext.*
import com.drgayno.contactstracer.bt.entity.ObservableScanSession
import com.drgayno.contactstracer.bt.entity.ScanSession
...
class BluetoothRepository(
    ...
    private val scanCallback = object : ScanCallback() {
        ...
        override fun onScanFailed(errorCode: Int) {
            Log.e(ScanFailedException(errorCode))
    ...
    private val advertisingCallback = object : AdvertiseCallback() {
        ...
        override fun onStartFailure(errorCode: Int) {
            Log.e(AdvertisingFailedException(errorCode))
        }
    }
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                Log.d("GATT connected. Mac: ${gatt.device.address}")
                gatt.discoverServices()
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                disconnectFromGatt(gatt)
                Log.d("GATT disconnected. Mac: ${gatt.device.address}")
            }
        }
        ...
        override fun onCharacteristicRead(
            ...
            if (GATT_CHARACTERISTIC_UUID == characteristic?.uuid) {
                ...
                if (tuid != null) {
                    Log.d("GATT TUID found. Mac ${gatt.device.address}. TUID: $tuid")
                    ...
                    Log.e("GATT TUID not found on $mac")
                }
                disconnectFromGatt(gatt)
            }
        }
    ...
    private fun handleIosDevice(result: ScanResult) {
        if (!discoveredIosDevices.containsKey(result.device.address)) {
            ...
            discoveredIosDevices[result.device.address]?.let {
                ...
                Log.d("Device (iOS) ${it.deviceId} - RSSI ${result.rssi}")
            }
            return
        }
    }
    ...
    private fun dbCleanup() {
        if (System.currentTimeMillis() - prefs.getLastDbCleanupTimestamp() > 24.hoursToMilis()) {
            Log.d("Deleting data older than ${AppConfig.persistDataDays} days")
            val rows = db.deleteOldData()
            Log.d("$rows records deleted")
